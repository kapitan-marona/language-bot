...
async def usage_gate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # >>> ADDED: байпас — админские/ценовые формы и числовой ввод для доната
    if (ctx.user_data.get("adm_promo_mode")                                     # >>> ADDED
        or ctx.user_data.get("adm_price_mode")                                  # >>> ADDED
        or ctx.user_data.get("donation_waiting_amount")):                       # >>> ADDED
        return                                                                  # >>> ADDED

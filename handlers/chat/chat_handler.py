...
# Простой per-user lock без новых файлов и зависимостей
_user_locks = collections.defaultdict(asyncio.Lock)
class user_lock:
    def __init__(self, uid: int):
        self._lock = _user_locks[uid]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with user_lock(chat_id):  # сериализуем обработку для одного пользователя
        ...
        try:
            ...
            if update.message.voice:
                ...
                try:
                    # offload в поток, чтобы не блокировать event loop
                    ...
                    transcript = await asyncio.to_thread(_whisper_call, audio_path)
            ...
            # Таймаут, чтобы не зависать навечно на внешнем сервисе

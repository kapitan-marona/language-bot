...
def format_promo_status_for_user(profile: dict) -> str:
    """
    Единое сообщение о промокоде:
    - первая строка: 'Промокод <код>:'
    - дальше построчно детали (как раньше во 2-м сообщении)
    Если промокод не активирован — возвращаем короткое сообщение без шапки.
    """
    from datetime import datetime, timedelta, timezone
    ...
    if ptype in ("permanent", "english_only"):
        ...
        if norm in ("friend", "друг"):
            details = f"⏳ действует ещё {_human_time_left(left)}\n🌐 открывает все языки и возможности\n🕊️ без ограничений"
        elif norm == "0825":
            ...
            details = f"⏳ действует до конца месяца — ещё {left_days} {_plural_ru_days(left_days)}\n🌐 открывает все языки и возможности\n🕊️ без ограничений"
        else:
            details = f"⏳ действует ещё {_human_time_left(left)}\n🌐 открывает все языки и возможности\n🕊️ без ограничений"
    else:
        details = "ℹ️ тип промокода не распознан"

    # Добавляем шапку с кодом и возвращаем единый текст
    header = f"Промокод {code_used}:" if code_used else "Промокод:"
    return f"{header}\n{details}"

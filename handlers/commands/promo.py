...
async def promo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ok_act:
        save_user_profile(
            ...
            access_plan=profile.get("access_plan"),
            access_expires_at=profile.get("access_expires_at"),
            access_lang_policy=profile.get("access_lang_policy"),
            access_lang_slots=profile.get("access_lang_slots"),
            access_active_langs_json=profile.get("access_active_langs_json"),
        )
        await safe_reply(update, context, format_promo_status_for_user(profile, ui))
    ...
    reason_map = {
        ...
        "already_used_by_you": ("⚠️ Вы уже использовали этот промокод ранее." if ui == "ru"
                                else "⚠️ You have already used this promo code."),

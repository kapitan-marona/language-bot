...
from state.session import user_sessions  # NEW: чтобы писать нуджи в историю

NUDGE_INTERVAL_DAYS = 14
BATCH_LIMIT = 200  # предохранитель
MAX_HISTORY_LENGTH = 40  # как в chat_handler
...
NUDGE_TEXTS = {
    "ru": [
        ...
        "Хей! У меня проблема 😂 — в голове застряла песня. Целый день играет «Du, du hast…» Rammstein 🤘 Знаешь способы, как выкинуть песню из головы?",
...
async def run_nudges(bot: Bot, chat_ids: Iterable[int], *, dry_run: bool = False) -> Tuple[int, int]:
    ...
    for chat_id in chat_ids:
        ...
        if dry_run:
            # в dry-run историю не трогаем
            continue

        try:
            await bot.send_message(chat_id=chat_id, text=text)
            # отметим отправку нуджа в профиле
            save_user_profile(chat_id, nudge_last_sent=now.isoformat())

            # NEW: добавим нудж в историю, чтобы следующий ответ пользователя имел контекст

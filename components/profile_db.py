...
# NEW: список всех chat_id для рассылок
...
# --- Broadcast audience helpers (для админки) ---

def get_chat_ids_by_interface_lang(interface_lang: str) -> List[int]:
    """
    chat_id пользователей с заданным языком интерфейса (например 'ru' или 'en').
    """
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    try:
...
def get_chat_ids_active_last_days(days: int = 7) -> List[int]:
    ...
    try:
        ...
        # user_usage может ещё не быть создана (init_usage_db не запускался)
        return []
    finally:
        conn.close()
...
def get_chat_ids_active_last_days_by_interface_lang(days: int = 7, interface_lang: str = "en") -> List[int]:
    """
    Активные за N дней + язык интерфейса.
    """
    days = int(days) if days else 7
    if days < 1:
        days = 1
